...
write.csv(file = "tmp/metareg_model_results_logratio.csv",model_results)
model_results_pi<-read.csv("tmp/metareg_model_results_logratio.csv")
...
write.csv(file = "tmp/metareg_model_results_logratio_m.csv",model_results)
model_results_m<-read.csv("tmp/metareg_model_results_logratio_m.csv")
...
model_results_pi %>%
  ...
  arrange(label) %>% 
  arrange(center) %>% 
  mutate(Percent.Change=-100*(exp(-center)-1)) %>% 
  mutate(Percent.Change=paste0(format(Percent.Change,digits=2) ,"%"))%>% 
  dplyr::select(label, DIC, summary_string, fixed_str, random_str,Percent.Change) %>%  # View
  print
...
model_results_m %>%
  ...
  mutate(Percent.Change=-100*(exp(-center)-1)) %>% 
  mutate(Percent.Change=paste0(format(Percent.Change,digits=2) ,"%"))%>% 
  dplyr::select(label, DIC, summary_string, fixed_str, random_str,Percent.Change) %>%
  # View
  print
