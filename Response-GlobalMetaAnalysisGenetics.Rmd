...
# Get path programatic
# mypath<-"~/extinction/Response-GenDivMetaAnalysis/"
library(here)
mypath <- here::here()  # This resolves to your project root (based on RStudio project or Git root)

# Set the path for Rstudio and knitting process
setwd(mypath)
...
Arithmetic mean Hedge's g = `r printmean(dfpici$HedgesG)` ±`r printCI(dfpici$HedgesG)` (95% CI).\
Weighted SE mean Hedge's g = `r clean(metazg$Weighted_mean)` ± `r clean(metazg$CI_half_width)`(95% CI) (z = `r clean(metazg$z)`, $P$ = `r metazg$p`).\
MCMCglmm mean (intercept) of Hedge's g = `r posteriormcmc(mcmcg)$summary`.

 

Arithmetic mean % $\pi$ = `r printmean(dfpici$Percent.Change)`% ± `r printCI(dfpici$Percent.Change)`% (95% CI).\
Weighted SE mean % $\pi$ = `r clean(metazperc$Weighted_mean)`% ± `r clean(metazperc$CI_half_width)`% (95% CI) (z = `r clean(metazperc$z)`, $P$ = `r metazperc$p`).\
MCMCglmm mean (intercept) of % $\pi$ = `r posteriormcmc(mcmcperc)$summary` .

Geometric mean % $\pi$ = `r printmean((exp(mean(dfpici$Log.Change)) - 1) * 100)`% ± `r clean((exp(printCI(dfpici$Log.Change)) - 1) * 100)`% (95% CI).\
Weighted SE geometric mean % $\pi$ = `r clean((exp(mean(metaztest(dfpici$Log.Change, dfpici$SE.Log.Change)$Weighted_mean)) - 1) * 100)`% ± `r clean((exp(mean(metaztest(dfpici$Log.Change, dfpici$SE.Log.Change)$CI_half_width)) - 1) * 100)`% (95% CI).\

*For allelic richness* $M$\

Average Hedge's g = `r printmean(dfmci$HedgesG)` ±`r printCI(dfmci$HedgesG)` (95% CI).\
Weighted SE mean Hedges'g = `r clean(mmetazg$Weighted_mean)` ± `r clean(mmetazg$CI_half_width)`(95% CI) (z = `r clean(mmetazg$z)`, $P$ = `r mmetazg$p`).\
MCMCglmm mean (intercept) of Hedges'g = `r posteriormcmc(mmcmcg)$summary`.

 

Arithmetic mean % $M$ = `r printmean(dfmci$Percent.Change)`% ± `r printCI(dfmci$Percent.Change)`% (95% CI).\
Weighted SE mean % $M$ = `r clean(mmetazperc$Weighted_mean)` ± `r clean(mmetazperc$CI_half_width)`(95% CI) (z = `r clean(mmetazperc$z)`, $P$ = `r mmetazperc$p`).\
MCMCglmm mean (intercept) of % $M$ = `r posteriormcmc(mmcmcperc)$summary`.

Geometric mean % $\pi$ = `r printmean((exp(mean(dfmci$Log.Change)) - 1) * 100)`% ± `r clean((exp(printCI(dfmci$Log.Change)) - 1) * 100)`% (95% CI).\
Weighted SE geometric mean % $\pi$ = `r clean((exp(mean(metaztest(dfmci$Log.Change, dfmci$SE.Log.Change)$Weighted_mean)) - 1) * 100)`% ± `r clean((exp(mean(metaztest(dfmci$Log.Change, dfmci$SE.Log.Change)$CI_half_width)) - 1) * 100)`% (95% CI).\
...
summary(mshawnopim)$solutions %>% 
  kable(caption = "Shaw meta-regression summary excluding π and M.",
)
...
model_results %>%
  ...
  kable("latex", booktabs = TRUE, longtable = TRUE,
        escape = TRUE,
        caption = "Percentage diversity π meta-regression model comparison.",
        col.names = c("label", "DIC", "summary", "fixed effects", "random effects"),
        align = c("l", "r", "l", "p{5cm}", "p{5cm}")) %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"))
...
model_results %>%
  ...
  kable("latex", booktabs = TRUE, longtable = TRUE,
        escape = TRUE,
        caption = "Percentage diversity π (delta SE) meta-regression model comparison.",
        col.names = c("label", "DIC", "summary", "fixed effects", "random effects"),
        align = c("l", "r", "l", "p{5cm}", "p{5cm}")) %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"))
...
model_results %>%
  ...
  kable("latex", booktabs = TRUE, longtable = TRUE,
        escape = TRUE,
        caption = "Log ratio diversity π meta-regression model comparison.",
        col.names = c("label", "DIC", "summary", "fixed effects", "random effects"),
        align = c("l", "r", "l", "p{5cm}", "p{5cm}")) %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"))
...
Distribution of positive and negative π div trajectory between two time points. Using Hedge's g for significance using two sided test and family-wise Bonferroni $P$-value correction 0.05/number of tests. The number of significant tests is as follows: Non-significant `r table(dfpici$pvalue_color)[2]`. Declining `r table(dfpici$pvalue_color)[1]`. Increasing `r table(dfpici$pvalue_color)[3]`.
...
```{r fig:drift_pi_noise ,echo=FALSE, warning=FALSE, eval=T,fig.width=7,fig.height=3.5, fig.cap="Expected population bottleneck and drift given observed π%. \\label{fig:Drift_panmictic_noise_grid}"}
