...
dmoi<-
  dmoi %>%
  mutate(Diversity.Metric = ifelse(Diversity.Type %in% c("A","AR"), "M", "π" ))
...
## Text S1: Subset dataset to robust and comparable genetic diversity metrics (π and S).

### Summary of numbers from the original dataset

Metrics in Shaw et al (2025) are of course known to population genetics, but across the literature there are a lot of variations, and some metrics may not be comparable across studies.

There are a number of metrics, not all of which are expected to change negatively or positively\
...
These were further grouped in four categories\
(definitions from Authors)\
...
-   We focus on time intervals below 30 years. The studies over 100 years are rare, `r table(d$Interval>100)[2]` or `r format(100*table(d$Gen.interval>100)[2]/nrow(d), digits=3)`%, and the majority of those, `r round(100*(table(d$Statistical.Method, d$Interval>100)/cbind(table(d$Statistical.Method),table(d$Statistical.Method)))[3,2])`%, are coalescent-based. And if scaled to generations, only `r length(unique(na.omit(filter(dfpi, Gen.interval > 30)$Binomen.rotl)))` species had time intervals higher than 30 generations.

```{r pi_kingdom_count, echo=F}

# dfpi$Kingdom %>%
#   table() %>%
  # as.data.frame() %>%
  # setNames(c("Kindom", "# species")) %>%
  # kable(.,caption="Number of observations by Kingdom")

table(dfpi$Kingdom,dfpi$Phylum ) %>% t() %>%
  # as.data.frame() %>%
  # setNames(c("Kindom", "# species")) %>%
  kable(.,caption="Number of observations by Kingdom and Phylum")

```

-   The number of observations by Kingdom and Phylum mostly show power to study insects, vertebrates, and flowering plants.
...
-   Number of observations of domesticated species:\
    Domesticated `r domesticated` observations (% `r 100*domesticated/nrow(d)`). Domesticated/pathogens/pests `r domesticatedorpests` observations (% `r 100*domesticatedorpests/nrow(d)`).
...
Total number `r nrow(dfpi)+nrow(dfm)` of which `r 100*nrow(dfpi)/(nrow(dfpi)+nrow(dfm))`% are $\pi$. The final subset is `r 100*(nrow(dfpi)+nrow(dfm))/nrow(d)`% of the original dataset.
...
ggplot(dmoip)+
  ...
  scale_color_manual(values = c("M"="#8B008B" ,"π"="#468B00" ) )+
...
Mean Percentage change (removing 1% outliers) = `r printmean(dfpici$Percent.Change[dfpici$Percent.Change > quantile(dfpici$Percent.Change,0.005) & dfpici$Percent.Change < quantile(dfpici$Percent.Change,0.995)])`%\
Range Percentage change (removing 1% outliers) = `r printranges(dfpici$Percent.Change[dfpici$Percent.Change > quantile(dfpici$Percent.Change,0.005) & dfpici$Percent.Change < quantile(dfpici$Percent.Change,0.995)])`%\
...
Mean Percentage change (removing 1% outliers) = `r printmean(dfmci$Percent.Change[dfmci$Percent.Change > quantile(dfmci$Percent.Change,0.005) & dfmci$Percent.Change < quantile(dfmci$Percent.Change,0.995)])`%\
Range Percentage change (removing 1% outliers) = `r printmean(dfmci$Percent.Change[dfmci$Percent.Change > quantile(dfmci$Percent.Change,0.005) & dfmci$Percent.Change < quantile(dfmci$Percent.Change,0.995)])`%\

```{r check_signi_variation_subset, eval=T, warning=F, echo=FALSE,fig.width=4,fig.height=6, fig.cap="Distribution of effect sizes (g) and raw genetic diversity changes (%)."}
...
ggplot()+
  ...
  scale_fill_manual(values = c("M"="#8B008B" ,"π"="#468B00" ) )+
...
Hedges' g and genetic diversity change in percentage are correlated since raw data is used to calculate g, although with a lot of noise (Pearson's r=`r cor.test(dmoip$HedgesG,dmoip$Percent.Change)$estimate`). The extremest values in $\pi$, which comprises the majority of the data however, typically do not coincide. We can see that of the 5% top $\pi$ values that decline based on Hedge's g (`r sum(declinetablepi[2,])`), only a small fraction are also among the top raw % values of decline (`r clean(100*declinetablepi[2,2]/sum(declinetablepi[2,]))`%). This is however the case of $M$ (`r clean(100*declinetablem[2,2]/sum(declinetablem[2,]))`%). Figure \ref{fig:raw} shows the raw average and estimated effect sizes in the comparision of Hedges' g and raw values of change (colors indicate significance, see section below).
...
We first attempt to reproduce the main result of average negative effect size, which we assume was correctly calculated by the authors, as positive control. We aim to reproduce the effect size estimate of Hedge's g=-0.11 but also re-run analyses with actual raw % change in genetic diversity $\pi$ and $M$.
...
*For Hedges' g of diversity* $\pi$\
Arithmetic average Hedge's g = `r printmean(dfpici$HedgesG)` ±`r printCI(dfpici$HedgesG)` (95% CI).\
Classic weighted mean Hedge's g = `r clean(metazg$Weighted_mean)` ± `r clean(metazg$CI_half_width)`(95% CI) (z = `r clean(metazg$z)`, $P$ = `r metazg$p`).\
MCMCglmm-based random meta-regression (following Shaw). Posterior (intercept) mean of Hedge's g =  `r posteriormcmc(mcmcg)$summary`.


Running MCMCglmm on the entire dataset: g = `r posteriormcmc(mcmcgall)`.


**For diversity** $\pi$  
Arithmetic average % $\pi$ = `r printmean(dfpici$Percent.Change)`% ± `r printCI(dfpici$Percent.Change)`% (95% CI).  
Classic weighted mean % $\pi$ = `r clean(metazperc$Weighted_mean)`% ± `r clean(metazperc$CI_half_width)`% (95% CI) (z = `r clean(metazperc$z)`, $P$ = `r metazperc$p`).  
...
Average Hedge's g = `r printmean(dfmci$HedgesG)` effect size, and average % $M$ = `r printmean(dfmci$Percent.Change)`%.

The problem with raw averages is that the variance of each estimate differs.\
A classic fixed-effect meta-analysis using inverse variance weighting and a z-score test yields:\
Weighted mean g = `r clean(mmetazg$Weighted_mean)` (z = `r clean(mmetazg$z)`, $P$ = `r mmetazg$p`)\
Weighted mean % $M$ = `r clean(mmetazperc$Weighted_mean)`% (z = `r clean(mmetazperc$z)`, $P$ = `r mmetazperc$p`)

Following Shaw, we use the R package **MCMCglmm** to fit a random meta-regression using generation interval and midpoint as covariates (as described in Shaw's Methods, including MCMC settings).\
Posterior intercept mean [95% HPD] of g = `r clean(mean(mmcmcg$Sol[, "(Intercept)"]))` [`r paste(clean(HPDinterval(mmcmcg$Sol[, "(Intercept)"])), collapse = " - ")`]\
Posterior intercept mean [95% HPD] of % $M$ = `r clean(mean(mmcmcperc$Sol[, "(Intercept)"]))`% [`r paste(clean(HPDinterval(mmcmcperc$Sol[, "(Intercept)"])), collapse = " - ")`%]



### Variations of meta-regression

```{r mcmcrobustness , eval=T, echo=F}
RERUN=F
...
if(RERUN){
  ...
  # Step 1: Define model specs
  model_specs <- tribble(
    ~label, ~fixed_effects, ~random_effects, ~prior,
    "m0Shaw", "Z.Year.Midpoint + Z.Gen.interval", c("PaperID"), list(R=list(V=diag(1), nu=0.002), G=list(G1=list(V=1, nu=0.002))),
    "m0Shaw.median", "M.Year.Midpoint + M.Gen.interval", c("PaperID"), list(R=list(V=diag(1), nu=0.002), G=list(G1=list(V=1, nu=0.002))),
    "mnull.priorreg", NULL, NULL, list(R = list(V = 1e-4, nu = 10)),
    "mnull.priorfixlie", NULL, NULL, list(R = list(V = 1e-5, nu = 100)),
    "mnull.priorweak", NULL, NULL, list(R = list(V = 1, nu = 0.002)),
    "m1", "Z.Year.Midpoint + Z.Gen.interval", c("PaperID"), NULL,
    "m2", "Z.Gen.interval + Z.Year.Midpoint", c("PaperID", "Kingdom", "Order", "Binomen.rotl"), NULL,
    "m3", "M.Year.Midpoint + M.Gen.interval", c("PaperID"), NULL,
    "m4", "M.Gen.interval + M.Year.Midpoint", c("PaperID", "Kingdom", "Order", "Binomen.rotl"), NULL,
    "m5", "M.Gen.interval + M.Year.Midpoint", c("PaperID", "Kingdom", "Order", "Binomen.rotl"), NULL,
    "m6", NULL, c("PaperID", "Kingdom", "Order", "Binomen.rotl"), NULL,
    "m7", NULL, c("PaperID", "Kingdom"), NULL,
    "m8", NULL, c("PaperID", "Kingdom", "Marker.Type"), NULL,
    "m9", NULL, c("PaperID", "Kingdom", "Marker.Type"), NULL,
    "m10", NULL, c("PaperID", "Kingdom", "Marker.Type"), NULL,
    "m9.priorreg", NULL, c("PaperID", "Kingdom", "Marker.Type"),
    list(
      R = list(V = 1e-4, nu = 10),
      G = list(
        G1 = list(V = 1e-4, nu = 10),
        G2 = list(V = 1e-4, nu = 10),
        G3 = list(V = 1e-4, nu = 10)
      )
    ),
    
    "m9.priorfixlike", NULL, c("PaperID", "Kingdom", "Marker.Type"),
    list(
      R = list(V = 1e-4, nu = 100), 
      G = list(
        G1 = list(V = 1e-5, nu = 100),
        G2 = list(V = 1e-5, nu = 100),
        G3 = list(V = 1e-5, nu = 100)
      )
    ),
    
    "m9.priorweak", NULL, c("PaperID", "Kingdom", "Marker.Type"),
    list(
      R = list(V = 1, nu = 0.002),
      G = list(
  ...
  model_results <- model_list %>%
    mutate(posterior = map(model, posteriormcmc)) %>%
    mutate(
  ...
  model_results<-read.csv(file = "tmp/metareg_model_results_hedge.csv")  %>%
    arrange(DIC)
}
...
model_results %>%
  ...
  kable("latex", booktabs = TRUE, longtable = TRUE,
        escape = TRUE,
        caption = "Effect size meta-regression model comparison.",
...
model_results %>%
  ...
  kable("latex", booktabs = TRUE, longtable = TRUE,
        escape = TRUE,
        caption = "Percentage π change meta-regression model comparison.",
        col.names = c("label", "DIC", "summary", "fixed effects", "random effects"),
        align = c("l", "r", "l", "p{5cm}", "p{5cm}")) %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"))
...
### Test per-study statistical differences, and whether there more or less positive trends?

Is the distribution of Hedge's g significantly different from zero? We follow a t-statistic distribution, which is the expected for Hedge's g . In addition we also conduct the test with a Gaussian distribution (which may be more akin to the meta-regression modeling, but this Normal distribution causes less significant results than t-distribution because is wider and thus more conservative). Figure \ref{fig:significanceplot} shows all studies ranked by $-log_{10}(p)$ of Hedge's g test per study, which has a substantial inflation. Figure \ref{fig:qq} shows the inflation of P-values assuming Hedges'g follow a t-distribution, making our detection of significant effects anti-conservative.
...
We find that while the meta-regression framework shows a negative average intercept, the majority of the data points are not significantly different from zero.

Distribution of positive and negative π div trajectory between two time points. Using Hedge's g for significance using two sided test and family-wise Bonferroni $P$-value correction 0.05/number of tests. The number of significant tests is as follows:\
Non-significant `r table(dfpici$pvalue_color)[2]`\
Declining `r table(dfpici$pvalue_color)[1]`\
Increasing `r table(dfpici$pvalue_color)[3]`
...
ggplot(dfpici)+
  ...
  # Tweaks
  ylab("Genetic diversity change (% π)")+
...
```{r fig:fig_change_pi_vs_hedge, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.height=3, fig.width=6,fig.cap="Relationship between raw values of diversity change π and Hedge's g values. \\label{fig:raw}" }
...
```{r, warning=FALSE, error=F, include=T, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.height=4, fig.width=3, fig.cap="Sample size (loci and individuals) and raw changes in π% genetic diversity."}
...
ggplot()+
  ...
  scale_size_continuous("Abs. π%",range = c(0,4))+
  scale_x_log10()+
  scale_y_log10()+
...
```{r fig:sample_sizes ,echo=FALSE, warning=FALSE,message=FALSE,error=FALSE, eval=T, echo=FALSE, fig.height=4, fig.width=6, fig.cap="Sample and maerker numbers across studies \\label{fig:sample_sizes}"}
p1<-
ggplot(dfpici)+
   geom_jitter(aes(y=Effective.N,
                x=factor(pvalue_color),
                color=factor(pvalue_color)
                 ),alpha=0.5
             )+
        stat_summary(aes(y=Effective.N,
                x=factor(pvalue_color),
                color=factor(pvalue_color)
                 ),alpha=1, size=2
             )+
   geom_violin(aes(y=Effective.N,
                x=factor(pvalue_color),
                color=factor(pvalue_color)
                 ),alpha=0.5
             )+
...
```{r fig:sample_sizes_past_present ,echo=FALSE, warning=FALSE,message=FALSE,error=FALSE, eval=T, echo=FALSE, fig.height=4, fig.width=6, fig.cap="Sample and maerker numbers across studies \\label{fig:samples_past_present}"}
tmp<-
dfpici %>% 
  pivot_longer(
...
It has become clear that the very extreme values of genetic diversity increases and decreases are owed to high standard error. A good rule of thumb is that standard error \< 5% are accurate measurements, while \>5% are highly uncertain, as the measurements we are aiming to achieve are in the same or smaller order of magnitude. We then look at enrichment of significant values in the more certain studies (we consider a 5% percentage change error as the limit of acceptable). We find a marginal enrichment of significant decline diversity cases $\pi$ and low error measurements (Odd=`r errortableenrichment$estimate`, Fisher's test $P$=`r errortableenrichment$p.value`) but not with significantly increasing $\pi$ studies (Odd=`r errortableenrichmentpos$estimate`, Fisher's test $P$=`r errortableenrichmentpos$p.value`).

### Test differences with domesticated.
...
Finally, hadn't we removed domesticated, pathogens, or pests, we would find an upward bias in genetic diversity change as there is an enrichment of domesticated species and significant positive trends in the original data (Odds= `r fisherdomesticated$estimate`, Odds= `r fisherdomesticated$p.value`).
...
```{r check_signi_populations_sides, eval=T, warning=FALSE, echo=FALSE,fig.width=4,fig.height=3.5}
...
```{r check_signi_repeatability, evaluate=T, warning=FALSE,echo=F, message=F, error=FALSE, fig.width=8,fig.height=8, fig.cap="Positive and negative cases of species with multiple observations. \\label{fig:multipleobservationspositivenegative}"}
...
```{r null plot, evaluate=F, warning=FALSE,echo=F, message=F, error=FALSE, fig.width=8,fig.height=8, fig.cap="Ranges of expected changes of diversity π% with biological sampling \\label{fig:biological_sampling_ranges}"}
...
allnullplot<-
  ...
  xlab("Percent change (%π)")+ylab("Null expectation of change (%π)")
...
```{r fig:biological_sampling ,echo=FALSE, warning=FALSE,message=FALSE,error=FALSE, eval=T, echo=FALSE,fig.width=5,fig.height=8, fig.cap="Expected maximum changes of diversity π% with biological sampling \\label{fig:biological_sampling}"}
...
Bootstrapping 1000 times one simulation per paper gives a range of average π% change= `r printranges(changesamples)`%.


The reason observations are very noisy is because the median sampling effort in Shaw is $n_0$=`r printmedian(as.numeric(dfpi$Early.Sample.N))` ,$n_1$=`r printmedian(as.numeric(dfpi$Recent.Sample.N))`, and the median number of loci is $L$=`r printmedian(as.numeric(dfpi$N.Loci))`. Fig. \ref{fig:SAMPLE_panmictic_noise_grid} shows the results of standard deviation of percent diversity change (%π) in a population of constant size sampled two times, in a grid of values from 5 individuals to 10,000 individuals, and a number of markers 1 to 1M. The median values of Shaw are marked in green, and indicate that the median error rate is over 10%, just as we see in the data.

```{r minimum_sample_size,  echo=F, warning=F,error=F, eval=T, message=F, fig.width=5,fig.height=3.5, fig.cap="Expected noise in percent diversity change π% with different ssample sizes \\label{fig:null_sampling}"}
# sims<-read.csv("pi_grid_simulation_results_realistic_loci.csv")
...
ggplot(testsims)+
  ...
  ylab("SD percent diversity change (%π)")+
...
myplot <-
  ...
  ylab("Percent diversity change (π%)") +
  scale_x_continuous(
    breaks = -4:-0,
    limits = c(-4, 0),
    labels = TeX(sprintf("$10^{%d}$", abs(-4:0)))
  ) +
  ggrepel::geom_text_repel(
...
```{r fig:SAMPLE_panmictic_noise_grid ,echo=FALSE, warning=FALSE, eval=T, warning=FALSE, echo=FALSE,fig.width=7,fig.height=3.5, fig.cap="Expected immediate population bottleneck given observed π% with noise \\label{fig:SAMPLE_panmictic_noise_grid}"}

plot_grid(myplot,myplot2)

```

*No noise*  <!-- Results (without error sampling): n = `r length((res$Pop.Bottleneck[res$pvalue_color== -1]))` observartions.\ --> Median = `r printmedian(res$Pop.Bottleneck[res$pvalue_color== -1])` individuals.\
...
*Noise*  <!-- n = `r length((datsim$Pop.Bottleneck[datsim$pvalue_color== -1]))` observations modeled.\ --> Median = `r printmedian(datsim$Pop.Bottleneck[datsim$pvalue_color== -1 & datsim$Pop.Bottleneck!=10000])` individuals.\
...
```{r fig:MAR_panmictic_noise ,echo=FALSE, warning=FALSE, eval=FALSE,fig.width=7,fig.height=3.5, fig.cap="Expected immediate population bottleneck given observed M%. \\label{fig:Immediate_panmictic_noise_grid}"}
SAVEPLOTS=F
...
*No noise* (assume the case of $N_0$=10000)  Median = `r printmedian((res$Pop.Bottleneck[res$pvalue_color==-1]))`% of original population\
...
*Noise* (assume the case of $N_0$=10000)  Median = `r printmedian((datsim$Pop.Bottleneck[datsim$pvalue_color==-1]))`% of original population\
...
```{r fig:drift_pi_noise ,echo=FALSE, warning=FALSE, eval=FALSE,fig.width=7,fig.height=3.5, fig.cap="Expected population bottleneck and drift given observed π%. \\label{fig:Drift_panmictic_noise_grid}"}
...
```{r create_GDAR ,echo=FALSE, warning=FALSE,fig.width=4,fig.height=3.5, fig.cap="Expected immediate geographic area loss given observed π%. "}
...
```{r fig:GDAR ,echo=FALSE, warning=FALSE,fig.width=7,fig.height=3.5, fig.cap="Expected immediate geographic area loss given observed π%.  \\label{fig:GDAR_panmictic_noise_grid}"}
...
Both spatial contraction and short drift. This is the most complicated scenario as we do not have a direct simple mathematical method. Instead, we have pre-computed scenarios of a solved numerical method we call WFmoments, that can predict the non-equilibrium genetic diversity of any meta-population system over time. We also only have solutions for average genetic diversity π. In this approach, then we created a number of loss area scenarios in a system of 100 demes in a 2D lattice, losing demes in an edge contraction manner. We then inverse model the area loss corresponding to the matched π change simulated and the observed π changes. We can use multiple functions, although they give qualitatively similar results. We decide for a Gaussian kernel, where the distance between the observed and simulated π changes exponentially decreases.
...
```{r fig:WFmoments_time_noise ,echo=FALSE, warning=FALSE,fig.width=7,fig.height=3.5, fig.cap="Expected immediate geographic area loss given observed π%. \\label{fig:WFmoments}"}
...
```{r fig:popgrowth, echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE,fig.width=10,fig.height=3.5, fig.cap="Expected population growth given increases in π%. \\label{fig:increases}"}
...
```{r fig:increasedecrease, echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE,fig.width=3.5,fig.height=3.5, fig.cap="The shape of genetic diversity increases is slower than decreases. \\label{fig:increasedecrease}"}

# --- Parameters ----------------------------------------------------
N0        <- 1000                 # ancestral effective size
scenarios <- tibble(
  scenario = c("Decline: 1000 → 500", "Growth: 1000 → 1500"),
  N1       = c(500, 1500)
)
t_max <- 4 *N0*2                     

# --- Helper: fraction of π-equilibrium at generation t -------------
pi_frac <- function(N1, t, N0) {
  r <- N1 / N0                               # N1 / N0
  r + (1 - r) * (1 - 1 / (2 * N1))^t         # π_t / π_0
}
...
ggplot(plot_df, aes(x = t, y = (pi_over_pi0-1)*100, colour = scenario)) +
  ...
  labs(
    ...
    y       = "Percent change diversity (%π)",
    # y       = expression(pi[t] / pi[0]),
    colour  = "Scenario"#,
    # title   = "Neutral recovery/loss of nucleotide diversity\nafter instantaneous population size change"
  ) +
  # ylim(0, 1.6) +                      # room for the 1.5 asymptote
  theme_minimal(base_family = "Times New Roman")+
...
```{r fig:FST ,echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE,fig.width=4,fig.height=3.5, fig.cap="Expected $F_{ST}$ to cause a inflation of π%.\\label{fig:fst} "}
...
```{r fig:WFmomentsfragmentation ,echo=FALSE, warning=FALSE, echo=FALSE, warning=FALSE,fig.width=4,fig.height=3.5, fig.cap="Expected noisy inflation of π% from area loss and fragmentation.\\label{fig:fragmentation} "}
...
## Text S6: Simple population bottleneck from LPI

```{r lpi, echo=FALSE, warning=FALSE, fig.width=4, fig.height=3.5, fig.cap="Res loss from geographic area reduction on π%.\\label{fig:fragmentation}"}
...
A thought experiment back-calculation of genetic diversity loss would be 
